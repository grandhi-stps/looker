view: leads_deals_test {
  derived_table: {
    sql: select distinct "Vendor Campaign".campaign_id as "Vendor Campaign ID",
               ...
               full join (select x.date_key,x.yearqtr,
                x.cal_month,
                x.month_name,
                x.cal_week, x.date,to_date(to_char(y.created_time,'yyyy-mm-dd'),'yyyy-mm-dd') "created_time"
from
(select date_key,yearqtr,
                cal_month,
                month_name,
                cal_week,date from xamplify_test.xa_date_dim where cal_year in(2019,2020))x
left  outer join
(select "Campaign Deal Reg".created_time, cast ((split_part("Campaign Deal Reg".created_time::text , '-',1)||split_part("Campaign Deal Reg".created_time::text , '-',2)
 ||left(split_part("Campaign Deal Reg".created_time::text , '-',3),2)) as int) as key
from xamplify_test.xa_campaign_deal_registration_d "Campaign Deal Reg"
where extract(year from created_time) in(2019,2020))y
on (x.date_key=y.key)
order by 1 asc

)
            "Date" on  to_date(to_char("Campaign Deal Reg".created_time,'yyyy-mm-dd'),'yyyy-mm-dd')
                = "Date".date
